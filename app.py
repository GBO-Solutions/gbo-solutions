...
with col2:
  ...
  with c2:
    with open('x.png','rb') as f_zip:
      st.download_button(label='Download image',data=f_zip,file_name='x.png',mime='image/png')
      #st.download_button(label=':lock: :red[download] :hammer_and_wrench:',data=f_zip,file_name='x.png',mine='image/png',key='Bsimplex',disabled=False,use_container_width=True)
  st.divider()
  # B-F-G-S
  c1,c2,c3 = st.columns([5,4,5])
  with c2:
