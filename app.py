...
with col1:
  if not st.session_state['show_simplex_text']: #*#
    ...
    st.markdown(f"""<div style='height: 650px; overflow-y: auto; background-color: white; padding: 1rem; border: 1px solid #ccc; border-radius: 8px; text-align: justify; color: red; font-size: 16px;'>{html_content}</div>""", unsafe_allow_html=True)
