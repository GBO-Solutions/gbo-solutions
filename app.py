...
with col2:
  ...
  with c2:
    with open('GBOsimplex.zip','rb') as f_zip:
      st.download_button(label=':lock: :red[download] :hammer_and_wrench:',data=f_zip,file_name='GBOsimplex.zip',mine='application/zip',key='Bsimplex',disabled=False,use_container_width=True)
  st.divider()
  # B-F-G-S
  c1,c2,c3 = st.columns([5,4,5])
  with c2:
