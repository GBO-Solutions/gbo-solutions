...
with col1:
  ...
  if not st.session_state['show_simplex_text']: #*#
    ...
    st.header('2. Theory of Simplex')
    st.subheader('2.1. Standard Form of a Linear Programming Problem')
    st.write('Linear programming is a mathematical optimization methodology aimed at determining the best possible value of a linear objective function, given a set of linear constraints. Such problems can be precisely described using systems of equations and inequalities.')
    st.write('The general standard form of a linear programming problem is formulated as follows:')
    st.write('where,')
    st.write('* the variables x1,x2,...,xn are called decision variables,')
    st.write('* the constants cj are the coefficients of the objective function,')
    st.write('* the coefficients aij define the linear constraints of the problem,')
    st.write('* and bi represent the right-hand side values of the constraints.')
    st.write('This formulation is well-suited for applying the Simplex method, as it enables a systematic transformation of the problem into tableau format and facilitates organized data processing.')
    st.text('-----')
    #st.markdown( #*#
    #    """
    #    
